...
(function () {
    ...
    function hideAuth() {
        ...
        if (user && userNameDisplay) userNameDisplay.textContent = user.name || 'Patient';
    }

    function showAuthError(msg) {
        if (authError) { authError.textContent = msg; authError.style.display = ''; }
    }
    ...
    if (authLoginTab) authLoginTab.addEventListener('click', () => {
        if (switchToLogin) switchToLogin.click();
    });
    if (authRegisterTab) authRegisterTab.addEventListener('click', () => {
        if (switchToRegister) switchToRegister.click();
    });
    ...
    if (logoutBtn) logoutBtn.addEventListener('click', () => {
        API.logout();
        localStorage.removeItem('medvault_medicines');
        localStorage.removeItem('medvault_checkups');
        localStorage.removeItem('medvault_documents');
        showAuth();
    ...
    formUploadDoc.addEventListener('submit', async (e) => {
        ...
        const saveDoc = async (fileData, fileName) => {
            try {
                await API.addDocument({ title: name, category, date, notes, fileName: fileName || '', fileData: fileData || '' });
                closeUploadModalFn();
                renderDocuments();
                renderDashboard();
    ...
    window.MedVault.viewDoc = async function (id) {
        try {
            ...
            showToast('error', 'Error', 'Could not load document.');
        }
    };

    window.MedVault.deleteDoc = async function (id) {
    ...
    window.MedVault.deleteMed = async function (id) {
    ...
    window.MedVault.takeDose = async function (id) {
        const meds = loadData(STORAGE_KEYS.medicines);
        ...
        try {
            const updated = await API.takeDose(id);
            renderStock();
            renderMedicines();
            renderDashboard();

            const newRemaining = updated.totalQuantity - updated.dosesTaken;
            const percent = (newRemaining / updated.totalQuantity) * 100;
    ...
    window.MedVault.restockMed = async function (id) {
        const amount = prompt('How many units are you restocking?');
        if (!amount || isNaN(amount) || parseInt(amount) <= 0) return;
        try {
            const updated = await API.restockMedicine(id, parseInt(amount));
            renderStock();
            renderMedicines();
            renderDashboard();
            showToast('success', 'Restocked', `${updated.name}: added ${amount} units.`);
        } catch (err) {
            showToast('error', 'Error', err.message);
        }
    };
    ...
    formAddCheckup.addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        try {
            await API.addCheckup({ type, doctor, lastDate, interval: parseInt(interval), notes });
            closeCheckupModalFn();
            renderCheckups();
            renderDashboard();
    ...
    window.MedVault.deleteCheckup = async function (id) {
    ...
    if (API.isLoggedIn()) {
        hideAuth();
        API.syncAll().then(() => initApp()).catch(() => initApp());
    } else {
        showAuth();
    }
